...
## Correctif post-Lot 1 — live API en mode dégradé

### Problème constaté en production
- Requêtes répétées `GET /api/race/tour-de-france/2026/stage/14/live/` journalisées en 404 lorsque l'étape ou la session live locale n'existe pas encore.

### Décision prise
- L'endpoint de polling live doit retourner une réponse JSON locale `available=false` en HTTP 200 si la stage/session n'est pas encore disponible localement. Cela évite les 404 bruyants et respecte le fonctionnement dégradé sans fournisseur externe.

### Fichiers modifiés
- `live/api.py`
- `tests/test_live_api_degraded.py`
- `docs/refactor/status.md`

### Tests ajoutés
- `tests/test_live_api_degraded.py` couvre une étape absente localement et une session live absente localement.

### Commandes exécutées
- `ruff check live/api.py tests/test_live_api_degraded.py` : OK.
- `pytest -q tests/test_live_api_degraded.py` : échec initial dû à `ALLOWED_HOSTS` dans le test, corrigé avec `testserver`.
- `pytest -q tests/test_live_api_degraded.py` : OK, 2 passed, 9 warnings de dépréciation Django/Python sans échec.
- `ruff check .` : OK.
- `python manage.py check` : OK.
- `python manage.py makemigrations --check` : OK, aucune migration manquante.
- `pytest -q` : OK, 8 passed, 9 warnings de dépréciation Django/Python sans échec.
